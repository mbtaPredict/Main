...
```
This will start running the file. First, the Hubway data will load. This will take around a minute. Next, input the date and time that you would like data for. You can input any date or time. The histrocial data that we have is for Hubway rides during 2011, 2012, and 2013. If the input is a date and time for which we have historical data, the GUI displays the historical data. There are two types of predictions that can be returned, an iminent predction, and a distant predction. Each of these predictions uses a different alogorithm to predict Hubway ridership. The iminent prediction makes use of a weather forcast to predict ridership, while the distant prediction does not. 
